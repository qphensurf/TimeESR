...
CONTAINS
   ...
   subroutine Hamiltonian (N, lambda, Delta, Eigenvalues, H, Ss, runs, Nm)
   implicit none
...
! exists on the running folder
   ...
   print *, 'States written in file:  ', Name_output
   print *, ' '
  endif !end of diverting if Name_output exists
! We go ahead and write the ouput even if Name_output comes from a previous run
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!                                                                                                     !
...
! Plotting the spin distribution for the first Nplot states 
   ...
   print *, ' '
    ...
    do i = 1, Nplot
      ...
      write (unit_spinoutput,*) 'State', i, (W(i)-W(1))*Hartree/GHz, 'Spin^2:', &
        real(spin2(i)), 'Spin:', 0.5*(sqrt(1+4*real(spin2(i)))-1)
    enddo
      close (unit_spinoutput)
