...
CONTAINS
   subroutine reading_input ( Nfreq, Ntime, t0, t1, Amplitude_seq, t_seq, stept,  &
       ...
       N_int, GammaC, Cutoff, redimension, &
   ...
   real (q) :: tolerance 
   logical :: runs, population, spindyn, redimension
...
! take 500 periods from the last point in time (Ntime)
...
! Compute DC current like the mean value between max and min
      period =2._q*pi_d/Freq_seq (1,1)
      stept = (time(2)-time(1))
      indexperiod = int(period/stept)

! take fifty periods to avoid edge effects
      p=50
      l=0
