...
cd $rundir
numdifffreqpoints=`echo "$freqdiff" | wc -w`

for dfreq in $freqdiff
do
mkdir $dfreq
cd $dfreq
...
for freq in `LC_ALL=C seq $freqstart $dfreq $freqend`
do
cat >TimeESR.input <<@@
*****************************************************************
******** Please keep this FORMAT, including seperators **********
...
*****************************************************************
@@
echo "Differential frequency::" $dfreq
...
# Plot the results
python plot_SpectraESR_compare.py $numdifffreqpoints $freqdiff SpectraESR.dat CompareDiffFreq.png
xdg-open CompareDiffFreq.png &
